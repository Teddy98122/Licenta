...
public class DeleteAbonat {
    ...
    {
        Parent DeleteScene = FXMLLoader.load(getClass().getResource("sampleDeleteAboant.fxml"));
    ...
    public void stergere(ActionEvent event) throws Exception {
        try {
            ...
            List<Abonat> list = sesionServ.createQuery("from Abonat").getResultList();
            for (Abonat tempAbn : list) {
                String Client_ID_str = Integer.toString(tempAbn.getClient_ID());
                if (Client_ID_str.equals(Client_ID.getText())) {
                    sesionServ.delete(tempAbn);
                }
            }
